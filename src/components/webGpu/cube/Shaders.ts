...
export const texturedCubeWGSL = /* wgsl */ `struct Uniforms {
    modelViewProjectionMatrix : mat4x4f,
  }
  @binding(0) @group(0) var<uniform> uniforms : Uniforms;
  ...
  fn fs_main(
    ...
    let textureColor = textureSample(myTexture, mySampler, fragUV);
    // 텍스처 색상과 위치 기반 색상을 혼합
    return mix(textureColor, fragPosition, 0.3);
  }
  `;
